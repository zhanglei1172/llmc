...
@ALGO_REGISTRY
class GPTAQ(BaseBlockwiseQuantization):
    def __init__(
    ...
    def add_quant_config(self):
        ...
        self.alpha = special_config.get('alpha', 0.25)
    ...
    def get_original_out(self, block):
        ...
        if self.block_idx == 0:
            self.ori_out = [x.cpu() for x in self.block_forward(block)]
            block = block.cpu()
        else:
            next_ori_out = [x.cpu() for x in self.block_forward(block, self.ori_out)]
            block = block.cpu()
            del self.ori_out
            gc.collect()
            torch.cuda.empty_cache()
            self.ori_out = next_ori_out
    ...
    def block_opt(self, block):
        ...
        self.ori_input_feat = defaultdict(list)
        handles = []
        for name in input_feat_modules:
            handles.append(
                input_feat_modules[name].register_forward_hook(
                    functools.partial(
                        super().cache_input_hook, name=name, feat_dict=self.ori_input_feat
                    )
                )
            )
    ...
    def layer_transform(self, layer, name):
        ...
        W, H, P = self.process_hessian_and_weights(layer, name)
        self.update_layer_with_transformed_weights(layer, W, H, P, name)
    ...
    def process_hessian_and_weights(self, layer, name):
        ...
        del self.layers_cache[name]['H'], self.layers_cache[name]['dXXT']
        ...
        P = self.alpha * ((dXXT @ H.T).triu_(diagonal=1)) @ H
        return W, H, P

    def update_layer_with_transformed_weights(self, layer, W, H, P, name):
        Losses = torch.zeros_like(W)
        tmp = torch.zeros_like(W)

        self.weight_transform(W, H, P, Losses, tmp)
    ...
    @torch.no_grad()
    def weight_transform(self, W, Hinv, P, Losses, tmp):
        for i1 in range(0, self.n_nonout, self.blocksize):
            i2 = min(i1 + self.blocksize, self.n_nonout)
            count = i2 - i1
            W1, Hinv1, P1 = W[:, i1:i2].clone(), Hinv[i1:i2, i1:i2], P[i1:i2, i1:i2]
            tmp1, Err1, Losses1 = (
                torch.zeros_like(W1),
                torch.zeros_like(W1),
                torch.zeros_like(W1),
    ...
    def add_batch(self, layer, name, inp, out):
        ori_inp = self.ori_input_feat[name][self.cnt]
        world_size = int(os.environ['WORLD_SIZE'])
        if len(inp.shape) == 2:
            inp = inp.unsqueeze(0)
            ori_inp = ori_inp.unsqueeze(0)
        tmp = inp.shape[0]
        if isinstance(
            ...
            if isinstance(layer, RotateLinear):
                ...
                ori_inp = layer.rotater.rotate(ori_inp)
            if len(inp.shape) == 3:
                inp = inp.reshape((-1, inp.shape[-1]))
                ori_inp = ori_inp.reshape((-1, ori_inp.shape[-1]))
            inp = inp.t()
            ori_inp = ori_inp.t()
        if isinstance(layer, nn.Conv2d):
            unfold = nn.Unfold(
            ...
            inp = inp.flatten(1)
            ori_inp = unfold(ori_inp)
            ori_inp = ori_inp.permute([1, 0, 2])
            ori_inp = ori_inp.flatten(1)
        ...
        ori_chunks = torch.chunk(ori_inp.to(inp.device), self.chunk_num, dim=1)

        self.layers_cache[name]['H'] *= self.layers_cache[name]['nsamples'] / (
            self.layers_cache[name]['nsamples'] + tmp
        )
        self.layers_cache[name]['dXXT'] *= self.layers_cache[name]['nsamples'] / (
            self.layers_cache[name]['nsamples'] + tmp
        )
        self.layers_cache[name]['nsamples'] += tmp

        for chunk, ori_chunk in zip(chunks, ori_chunks):
            chunk = math.sqrt(2 / self.layers_cache[name]['nsamples']) * chunk.float()
            self.layers_cache[name]['H'] += chunk.matmul(chunk.t())
            dX = math.sqrt(2 / self.layers_cache[name]['nsamples']) * ori_chunk.float() - chunk
